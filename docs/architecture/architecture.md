...
| **Publish fixture** | Build the landing fixture from the processed window, then **additively merge** it onto the previously published full-history fixture (fresh months win per-month; 1990→ history preserved). **Required previous** (merge-on), **collapse guard** (<50% of prior records), and **density guard** (median records/country vs advertised months) abort the run instead of shipping empty history. `months` is rewritten from record keys so the slider cannot advertise empty years. Override only with `--force`. See [runbook §1.6a](../ops/runbook.md) and `src/pipeline/lib/fixture-publish.ts` | `landing/src/data/fixture-snapshot.json` + Pages shards |
